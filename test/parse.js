...
var ACCELLEROMETER_DATA = Buffer.from([0x80, 0x09, 0x00, 0x20, 0x90, 0xc4, 0x00, 0xe4, 0xff, 0x6d, 0x00, 0x02, 0x03, 0x7e]);
var TEMP_DATA = Buffer.from([0xa0, 0x03, 0x00, 0x20, 0x91, 0x15, 0x56, 0x10]);
...
describe('Bean', function () {
  ...
  describe('Parse', function () {
    ...
    it('parses raw data', function (done) {
      bean.on('raw', function(buf, length, valid, command){
        expect(buf).to.eql(Buffer.from([0x20, 0x90, 0xc4, 0x00, 0xe4, 0xff, 0x6d, 0x00, 0x02]));
    ...
    it('parses serial on serial', function (done) {
      bean.on('serial', function(data, valid){
        expect(data).to.eql(Buffer.from([0x48, 0x65, 0x6c, 0x6c, 0x6f]));
        expect(valid).to.eql(true);
