...
Bean.prototype._onRead = function(gt){
  ...
  if(packetCount === 0){
    ...
    var crc16 = Buffer.from(crcString, 'hex');
...
Bean.prototype.send = function(cmdBuffer,payloadBuffer,done){
  ...
  var crc16Buffer = Buffer.from(crcString, 'hex');

  //GATT contains sequence header, gstBuffer and crc166
  var gattBuffer = Buffer.alloc(1 + gstBuffer.length + crc16Buffer.length);
...
Bean.prototype.unGate = function(done){
  this.send(commands.MSG_ID_GATING, Buffer.from([]), done);
}
...
Bean.prototype.requestAccell = function(done){
  this.send(commands.MSG_ID_CC_ACCEL_READ, Buffer.from([]), done);
};

Bean.prototype.requestTemp = function(done){
  this.send(commands.MSG_ID_CC_TEMP_READ, Buffer.from([]), done);
};
